// commands/sticker-alt.js - Alternative Sticker Maker
...
async function stickerAltCommand(sock, chatId, message) {
    try {
        ...
        const inputPath = path.join(TMP_DIR, `sticker_alt_${Date.now()}.webp`);
        ...
        try {
            ...
            const tempInput = path.join(TMP_DIR, `sticker_alt_input_${Date.now()}.${ext}`);
        ...
        console.error('[StickerAlt] Error:', error);
        await sock.sendMessage(chatId, { react: { text: '❌', key: message.key } });
    }
}

module.exports = stickerAltCommand;